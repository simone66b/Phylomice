...
setup <- list(
  miceMethod = c("phpp", "phppm", "pmm", "phpmm", "norm", "phnorm"),
            ...
            for (missDataMech in setup$missDataMech) {
              for (missDataLevel in setup$missDataLevel) {
                  ...
                  tryCatch({
                    ...
                    if (miceMethod == "phpp" || miceMethod == "phppm") {
                      psi <- cophenetic(tree)
                    }
...
generateMissingByMech <- function(data, missDataMech, missDataLevel) {
  if (missDataMech == "mcar") {
    ...
    miss <- generateMissing(data, perc)
    return(cbind(data[, 1:(ncol(data) - 1)], miss))
    ...
    lastCol <- ncol(data)
    data[, lastCol] <- ifelse(probs < runif(nrow(data)), NA, data[, lastCol])
    return(data)
  }
}
...
generateMissing <- function(data, perc, cols = ncol(data)) {
